...
(() => {
  ...
  const ALLOWED = new Set(['pfp.png','crab.png','jellyfish.png','fox.png','koala.png']);
  ...
  const getSaved = () => {
    try { return normalize(localStorage.getItem(AVATAR_KEY)); }
    catch { return 'pfp.png'; }
  };
  const setSaved = f => {
    try { localStorage.setItem(AVATAR_KEY, normalize(f)); } catch {}
  };
  const apply = f => { avatarImg.src = `./assets/icons/${normalize(f)}`; };

  const open  = () => { popover.hidden = false; };
  const close = () => { popover.hidden = true; };
  const toggle= () => { popover.hidden = !popover.hidden; };
