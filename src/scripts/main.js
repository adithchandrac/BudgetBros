(function () {
  ...
  /* demo notice, might remove later*/
...
// add/remove to local storage, change to json in future
...
// function allows user to filter their transactions
...
// finds and displays most recent transaction
...
// event handler for budget ranges
spans.forEach((span) => {
...
// function allows user to make or generate their budget
function budgetform(period) {
  ...
  document.getElementById("automate-budget").addEventListener("click", () => {
    ...
    if (limit <= 1) {
      //need users spending limit
      alert(
        "Please enter a spending limit greater than 1 to generate a sample budget"
      );
      return;
    }

    // sample budget alotment
    const pct = [
      1.0, // Spending Limit
      0.3,
      0.1,
